...
try{
    ...
    $sql="Select * From citizen,criminalrecord WHERE citizen.CitId=criminalrecord.CitizenId ";
...
(function() {
    ...
    var db = {
        ...
        loadData: function(filter) {
            return $.grep(this.clients, function(client) {
                return (!filter.FullName || client.FullName.indexOf(filter.FullName) > -1)
                    && (!filter.CitId || client.CitId === filter.CitId)
                    && (!filter.HomeAddress || client.HomeAddress.indexOf(filter.HomeAddress) > -1)
                    && (!filter.CMND || client.CMND.indexOf(filter.CMND) > -1)
                    && (!filter.Number || client.Number.indexOf(filter.Number) > -1)
        ...
        deleteItem: function(deletingClient) {
            ...
            alert(clientIndex);
            window.location.href="khoacongdan.php?id="+this.clients[clientIndex].ID;
        }
    ...
    db.clients = [
   		<?php while ($row=$query->fetch()): ?>
        {
                ...
                CitId:"<?php echo $row['CitId']; ?>",
                FullName:"<?php echo $row['FullName']; ?>",
                Birthday:"<?php echo $row['Birthday']; ?>",
                HomeAddress:"<?php echo $row['HomeAddress'] ?>",
                CMND:"<?php echo $row['CMND'] ?>",
