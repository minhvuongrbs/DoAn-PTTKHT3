    ...
    var db = {
        ...
        loadData: function(filter) {
            ...
            return $.grep(this.clients, function(client) {
                return (!filter.Name || client.Name.indexOf(filter.Name) > -1)
                    && (!filter["TemId"]|| client["TemId"] === filter["TemId"])
                    && (!filter["Số CMND"] || client["Số CMND"].indexOf(filter["Số CMND"]) > -1)
                    && (!filter["Địa chỉ tạm trú"] || client["Địa chỉ tạm trú"].indexOf(filter["Địa chỉ tạm trú"]) > -1)
                    && (!filter["Địa chỉ tạm vắng"] || client["Địa chỉ tạm vắng"].indexOf(filter["Địa chỉ tạm vắng"]) > -1)
                    && (!filter["Điện thoại"] || client["Điện thoại"].indexOf(filter["Điện thoại"]) > -1)
            });
        },
        ...
        loadDataBase: function(){
            <?php 
                    ...
                    $sql = "SELECT `TemId`, `CitizenID`, `AddressCurrent`, `AddressNew`,  citizen.FullName, citizen.CMND, citizen.Phone FROM `temporary`,`citizen` WHERE citizen.CitId = temporary.CitizenID";
            ...
            if(this.clients.length===0){
                ...
                this.clients.push(
                    {  
                        "TemId": "<?php echo $row['TemId'] ;?>",
                        "Name": "<?php echo $row['FullName'] ;?>",
                        "Số CMND": "<?php echo $row['CMND'] ;?>",
                        "Địa chỉ tạm trú": "<?php echo $row['AddressNew'] ;?>",
                        "Địa chỉ tạm vắng": "<?php echo $row['AddressCurrent'] ;?>",
                        "Điện thoại": "<?php echo $row['Phone'] ;?>",
                    }
                )
                <?php 
                    $i++;
